...
const std::pair<User *, bool>& Channel::getUser(std::string& nickname) const
{
...
bool Channel::containsUser(std::string& nickname) const
{
	return (getUser(nickname).first != NULL);
}

void Channel::promoteUser(std::string& nickname)
{
	if (containsUser(nickname))
		this->_users[nickname].second = true;
}
void Channel::demoteUser(std::string& nickname)
{
	if (containsUser(nickname))
		this->_users[nickname].second = false;
}

void Channel::broadcast(std::string& nickname, std::string& message) const
{
...
void Channel::announce(std::string& message) const
{
	...
	{
		if (it->second.first)
			send(it->second.first->getUserfd(), message.c_str(), message.size(), 0);
	}
}
